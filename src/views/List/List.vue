<template>
    <div class="section list">
        ...
        <div class="search-area">
            ...
            <input type="text" placeholder="검색어 입력" class="search-input" @change="input"/>
            <button type="button" class="btn-blue-sm" @click="clickSearch">검색</button>
        </div>

        <div class="table-wrap">
            <div class="table-info">
                <button type="button" class="btn-blue-sm" @click="moveRegister">등록하기</button>
            </div>
            <table>
                <colgroup>
                <col width="40px;"/>
                ...
                <tbody v-if="searchFilter.length!==0">
                    <tr v-for="(list,i) in searchFilter" :key="i">
                        ...
                        <td @click="detail(i)" class="underline">{{list.campaign_title}}</td>
                        <td>{{list.sns_type}}</td>
...
export default {
    ...
    methods : {
        ...
        deleteIdx(i){
            axios.delete(`http://3.36.103.222:80/scheduler/${this.campaignData[i].idx}`,{
            headers : {
    ...
    created() {
        axios.get('http://3.36.103.222:80/scheduler',{
            headers : {
              authorization : `Bearer ${localStorage.getItem('token')}`
            }
          })
